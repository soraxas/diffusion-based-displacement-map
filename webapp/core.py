...
def main():
    return redirect(
        "https://github.com/gxercavins/image-api/blob/master/README.md", code=302
    )
...
def rotate(angle, filename):
    ...
    if not -360 < angle < 360:
        return (
            render_template(
                "error.html", message="Invalid angle parameter (-359 to 359)"
            ),
            400,
        )
...
def flip(mode, filename):
    ...
    if mode == "horizontal":
        ...
        return (
            render_template(
                "error.html", message="Invalid mode (vertical or horizontal)"
            ),
            400,
        )
